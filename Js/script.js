...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const bookSliderWrapper = document.querySelector('.booksSlider .swiper-wrapper');

    // Book data with their image paths
    const books = [
        { title: 'The Gecko And The Echo', image: 'image/Echo.png' },
        { title: '100 Food 100 Food', image: 'image/Food.png' },
        { title: 'The Line Inside Lion', image: 'image/Lion.png' },
        { title: 'The Wobblysaurus Side', image: 'image/InSide.png' },
        { title: 'Nursery Rhymes Inside', image: 'image/Nursery.png' },
        { title: '100 Words 100 Words', image: 'image/Words.png' }
    ];

    // Function to render all books in the slider
    function renderBooks(bookList) {
        bookSliderWrapper.innerHTML = ''; // Clear existing slides
        bookList.forEach(book => {
            const slide = document.createElement('a');
            slide.classList.add('swiper-slide');
            slide.href = '#';
            slide.innerHTML = `<img src="${book.image}" alt="${book.title}">`;
            bookSliderWrapper.appendChild(slide);
        });
    ...
    // Initialize Swiper
    let swiper = new Swiper(".booksSlider", {
        loop: true,
        centeredSlides: true,
        autoplay: {
            delay: 9500,
    ...
    searchInput.addEventListener('click', () => {
        ...
        if (searchTerm) {
            ...
            if (foundBook) {
                alert(`Book "${searchTerm}" is available! Displaying it now.`);
                // Display only the found book
                renderBooks([foundBook]); // Render only the found book
                swiper.slideTo(0, 1000, true); // Move to the first slide (only slide available)
            } else {
                alert(`Book "${searchTerm}" is not available. Showing all books.`);
                // Show all books if not found
                renderBooks(books); // Render all books again
            }
        } else {
